...
/**
 * Abclass Administration Model
 *
 * This class model administration of site
 *
 * @package		ABClass
 * @subpackage	Models
 * @category	Models
...
public function materials_search($search,$limit,$start_from)
{
    ...
    {   
        ...
        if ($position !== FALSE)
        {
            //Увеличиваем на "1" счетчик материалов, удовлетворяющих запросу
            //(нужно для формирования элементов массива $total_result )
            $i++;
            
            //Начальная позиция вывода: 100 - на сколько символов до искомого запроса мы начинаем вывод), в любом случае
            //$begin_from будет содержать отрицательное число
            $begin_from = $position - $length - 100;                  

            /* Получаем нужную нам часть: строка; откуда начинаем; общая
            длина фрагмента. (так как $begin_from имеет отрицательное
            значение, то возвращаемая строка будет начинаться с
            $begin_from знака, считая с конца строки
			*/      
            $material['main_text'] = substr ($material['main_text'],$begin_from, 250);        

            /* Формируем массив со всеми найденными материалами в цикле ($i увеличивается при каждом подходящем материале,
            и мы получаем массив со всеми материалами, содержащими поисковый запрос, который для каждого подходящего
            материала содержит указанные ниже, нужные нам поля)
            */
            ...
            /* прибавляем "1" к переменной $i, так как нам нужно реальное количество удовлетворяющих
            запросу материалов, а нумерация у нас идет с нуля (мы сами задали начальное значение счетчика
            в "-1", увеличивая его при каждом подходящем материале)
			*/
        }           
    }

    // Объявляем массив (важно, вне цикла) - в нем будут формироваться
    //найденные материалы в соответствии с разбивкой pagination
    $msearch_results = array();
    
    // Для этого запускаем цикл: $start_from (с какого по счету найденного материала начинать вывод) передается
    // функции materials_search по ссылке, сформированной с помощью pagination в результатах поиска; $limit - ограничение
...
/* End of file administration_model.php */
/* Location: ./application/models/administration_model.php */