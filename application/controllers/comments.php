 ...
 * Abclass Comments
 *
 * This class controller comments of site
 *
 * @package		ABClass
    ...
    {                 
        ...
        if ($val_res == TRUE)
        {
             ...
             if ($entered_captcha == $this->session->userdata('rnd_captcha'))
             {
                  ...
                  //$address = "katja-fedorova@yandex.ru";
                  //$address = "";
...
public function edit_list($start_from = 0)
{
	...
	$data['help_desc'] = $this->helps_model->get_by('comments_edit_list');	
...
public function edit($comment_id = '')
{
    ...
    {
    	$data['help_desc'] = $this->helps_model->get_by('comments_edit');
...
public function update($comment_id = '')
{
   ...
   if (isset($_POST['update_button']))
   {
	   ...
	   if ($this->form_validation->run() == TRUE)
       {
		   ...
		   $data['help_desc'] = $this->helps_model->get_by('comments_update');

           $this->display_lib->admin_info_page($data);
       }
       else
       {    
            //формируем массив с данными о комментарии для подстановки в поля формы (те, что не прошли валидацию,
            //берутся из базы, а те, что прошли - из массива POST)
            $data = array();	             
            $data = $this->comments_model->get($comment_id);                    
		    $data['help_desc'] = $this->helps_model->get_by('comments_update_not_valid');
...
public function delete($start_from = 0)
{
    ...
    if ( ! isset($_POST['delete_button']))
    {
		...
		$data['help_desc'] = $this->helps_model->get_by('comments_delete');
    ...
    else
    {
        ...
        if ( ! isset($_POST['comment_id']))
        {
			...
			$data['help_desc'] = $this->helps_model->get_by('comments_delete_not_checked');
            $this->display_lib->admin_info_page($data);
        }
        ...
        {  
			...
			$data['help_desc'] = $this->helps_model->get_by('comments_deleted');
