...
/**
 * Abclass Administration
 *
 * This class controller administrative part of site
 *
 * @package		ABClass
...
public function index()
{
   ...
   // всего туров
   $data['tours_count']   = $this->materials_model->count_all('tours');

   // всего новостей
   $data['news_count']   = $this->materials_model->count_all('news');
   
   // всего отзывов
   $data['notes_count']   = $this->materials_model->count_all('notes');
   ...
   $data['help_desc'] = $this->helps_model->get_by('main_admin');
...
public function preferences()
{
   ...
   if (isset($_POST['save_button']))
   {
       ...
       if ($this->form_validation->run() == TRUE)
       {
		   ...
		   $data['help_desc'] = $this->helps_model->get_by('administration_preferences_sav');
       ...
       else
       {
			...
			$data = array();
			$data['help_desc'] = $this->helps_model->get_by('administration_preferences');
   ...
   //Кнопка "Сохранить настройки" не нажата: выводим просто форму с подставленными из базы данными
   //(данные напрямую берутся в виде, что не совсем по правилам)
   else
   {
   	   $data = array();
	   $data['help_desc'] = $this->helps_model->get_by('administration_preferences');
...
{
	$infostring = "";
	if($info == 'not_correct_user_pass')
	{
		$infostring = "Введен неверный пароль или имя пользователя.";
	}
    ...
    {
        $this->auth_lib->do_login($this->input->post('login'),md5($this->input->post('pass')));
    }
}
